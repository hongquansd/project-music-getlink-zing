...
{foreach from=$listsongs item=listsong name=listsong}
{if $smarty.foreach.listsong.last}
<div class="row bgmusic noborder">
{else}
<div class="row bgmusic">
{/if}
    <h3>
        <a href="{$baseurl}/song/{$listsong.id}">{$listsong.title}</a></h3>
    <p>
        
        <img src="{$imageurl}/ico-singer.gif" width="12" height="11" border="0"> {$listsong.singer}
            <span>
